...
router.post('/getlist', function(req, res, next) {
    ...
    signupschema.find({ "work": { $ne: null }, "time": { $lt: reqTime } }, { _id: 1, name: 1, year: 1, isNerd: 1, designation: 1, work: 1, time: 1 }, function(err, todos) {
        if (err) return next(err);
        ...
        if (todos[todos.length - 1] != undefined) {
          ...
          // send the time of last element when the list is not null
          lastRecordTime = todos[todos.length - 1].time;

        } else {

          // in case of null, send the 99 which will be later handled at client side
...
router.post('/filter', function(req, res, next) {
    ...
    if (req.query.year == undefined) {
        // removing year as a filter
        console.log("year is null");
        signupschema.find({ "work": { $ne: null }, "branch": { $in: req.query.branch } }, { _id: 1, name: 1, year: 1, isNerd: 1, designation: 1, work: 1, time: 1 }, function(err, todos) {
            if (err) return next(err);
    ...
    } else if (req.query.branch == undefined) {
        // removing branch as a filter
        console.log("branch is null");

        signupschema.find({ "work": { $ne: null }, "year": { $in: req.query.year } }, { _id: 1, name: 1, year: 1, isNerd: 1, designation: 1, work: 1, time: 1 }, function(err, todos) {
            if (err) return next(err);
    ...
    } else if (req.query.year.length > 0 && req.query.branch.length > 0) {


        console.log("year/branch is not null");

        signupschema.find({ "work": { $ne: null }, "year": { $in: req.query.year }, "branch": { $in: req.query.branch } }, { _id: 1, name: 1, year: 1, isNerd: 1, designation: 1, work: 1, time: 1 }, function(err, todos) {
            if (err) return next(err);
